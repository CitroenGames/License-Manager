...
app.post('/register', async (req, res) => {
  ...
  const hashedPassword = await bcrypt.hash(password, 10);
  db.run('INSERT INTO users (username, password, isAdmin) VALUES (?, ?, 0)', [username, hashedPassword], function (err) {
    if (err) {
...
app.post('/admin/generate', authenticateAdmin, (req, res) => {
  ...
  db.run('INSERT INTO license_keys (key, duration) VALUES (?, ?)', [licenseKey, duration], function (err) {
    if (err) {
      res.status(500).send({ message: 'Error generating license key' });
    } else {
      res.status(201).send({ message: 'License key generated', key: licenseKey });
    }
  });
});
...
db.all('SELECT id, username, expiration, isAdmin FROM users', [], (err, rows) => {
if (err) {
    ...
    db.get('SELECT * FROM users WHERE id=?', [decoded.id], (err, row) => {
      if (err || !row) {
        ...
        if (currentTime < expirationTime) {
          ...
          res.status(200).send({ message: 'Authenticated', hasTimeLeft: false });
        }
      }
    });
  } catch {
